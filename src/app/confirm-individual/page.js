...
export default function ConfirmIndividual() {
...
const executePrintSave = async () => {
  ...
  const StepCell = ({ row, col, children, bg = '#dddddd', color = '#fff' }) => (
    <Cell row={row} col={col} bg={bg} color={color} border="1px solid #fff" size="12px">
      {children}
    </Cell>
  );

  const ChangeButton = ({ row, col = '6 / 9', onClick }) => (
    <button
      onClick={onClick}
      style={{
        gridRow: row,
    ...
    <main className="app-shell">
      <div className="ipad-frame" style={{ overflowY: 'auto', overflowX: 'hidden' }}>
        <div
          ...
          style={{
            height: '207%',
            gridTemplateRows: 'repeat(62, 1fr)',
          }}
        >
...
>
  ...
  />
          ...
          <Cell row="9 / 10" col="3 / 39" bg="#000" color="#fff">注文内容</Cell>
          ...
          <Cell row="12 / 34" col="3 / 9" bg="#dddddd" />
          <Cell row="12 / 14" col="3 / 6" bg="transparent" border="none" color="#000">
            文字
          </Cell>
          ...
          {rows.map((row, index) => {
            ...
            const bottom = top + 2;
            const labelBg = index % 2 === 0 ? '#666666' : '#000000';
          ...
          <Cell row="44 / 46" col="9 / 39" size="14px">
            {orderData.options?.winter ? '防寒着への刺繍' : ''}
          </Cell>

          <LeftLabel row="46 / 49">備考欄</LeftLabel>
          <ChangeButton row="46 / 49" onClick={() => router.push('/step6')} />
          <Cell
            row="46 / 49"
            col="9 / 39"
            size="14px"
          ...
          <Cell row="50 / 52" col="3 / 15" bg="#000" color="#fff">料金</Cell>
          <Cell row="50 / 52" col="15 / 23" bg="#dddddd">単価</Cell>
          <Cell row="50 / 52" col="23 / 31" bg="#dddddd">数量</Cell>
          <Cell row="50 / 52" col="31 / 39" bg="#dddddd">金額(税込)</Cell>

          <Cell row="52 / 56" col="3 / 7" bg="#dddddd">基本料</Cell>
          <Cell row="52 / 54" col="7 / 15" bg="#dddddd">1ヶ所目/1行目</Cell>
          <Cell row="54 / 56" col="7 / 15" bg="#dddddd">1ヶ所目/2行目{'\n'}または2ヶ所目</Cell>
